...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if not files or files[0].filename == '':
            ...
            if conn:
                conn.close()
    return render_template_string(UPLOAD_TEMPLATE, result_msgs=result_msgs)
...
def query_data():
    ...
    return render_template_string(QUERY_TEMPLATE, 
                                ...
                                all_tables=TABLE_DISPLAY_NAMES,
                                search_term=search_term,
                                sort_field=sort_field,
                                sort_order=sort_order,
                                fields=fields,
                                all_columns=all_columns,
                                max=max,  # 添加max函数到模板上下文
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    // 监听排序方向选择，保证同字段只能选一个方向
    document.getElementById('sortOrdersSelect').addEventListener('addItem', function(e) {
        ...
        sortOrdersChoices.setChoiceByValue(Object.values(fieldMap));
    }, false);
...
function getSelectedSortOrders() {
    const sel = document.getElementById('sortOrdersSelect');
    // 只取ASC或DESC
    return Array.from(sel.selectedOptions).map(o => o.value.split('(')[0]).join(',');
}
...
function openEditDialog(idx) {
    ...
    document.getElementById('editForm').onsubmit = function(e) {
        ...
        fetch('/api/update_row', {
            ...
            if(res.success){ location.reload(); } else { alert('修改失败：'+res.msg); }
        });
    };
}
